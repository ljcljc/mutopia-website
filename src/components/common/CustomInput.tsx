  ...
  ({ label, error, rightElement, containerClassName = "", inputClassName = "", borderClassName = "", ...props }, ref) => {
    return (
      <div className="content-stretch flex flex-col gap-[8px] items-start relative w-full">
        ...
        {/* Input Container */}
        <div className={`bg-white h-[36px] relative rounded-[12px] shrink-0 w-full group ${containerClassName}`}>
            ...
            <div className="box-border content-stretch flex h-[36px] items-center px-[16px] py-[4px] relative w-full">
              <div className="basis-0 content-stretch flex grow items-center min-h-px min-w-px relative shrink-0">
                <input
                  ref={ref}
                  {...props}
                  className={`basis-0 font-['Comfortaa:Regular',_sans-serif] font-normal grow leading-[normal] min-h-px min-w-px relative shrink-0 text-[#4a3c2a] text-[12.25px] bg-transparent border-none outline-none placeholder:text-[#717182] w-full ${inputClassName}`}
                />
                {rightElement}
              </div>
            </div>
