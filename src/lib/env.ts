 ...
 * 获取环境变量值（带类型转换）
 */
type EnvKeys = keyof ImportMeta["env"];

export function getEnv(key: string, defaultValue?: string): string {
  return (import.meta.env[key as EnvKeys] as string) || defaultValue || "";
}
...
export function getEnvBoolean(
  ...
  const value = import.meta.env[key as EnvKeys] as string | undefined;
...
export function validateEnv(): void {
  ...
  required.forEach((key) => {
    if (!import.meta.env[key as EnvKeys]) {
      missing.push(key);
    }
  });
