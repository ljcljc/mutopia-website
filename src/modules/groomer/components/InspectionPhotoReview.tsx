      ...
      <div className="relative mx-auto h-full w-full overflow-hidden bg-black md:my-6 md:flex md:h-[calc(100vh-3rem)] md:max-h-[980px] md:max-w-[760px] md:flex-col md:rounded-[28px] md:bg-white md:text-[#4A3C2A] md:shadow-2xl">
        ...
        >
          <div className="touch-none select-none cursor-grab px-5 pb-4 pt-3 active:cursor-grabbing" onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
            ...
            <h3 className="font-comfortaa text-[26px] leading-[30px]">{title}</h3>
            <p className="mt-1 truncate font-comfortaa text-[14px] leading-[20px] text-white/65">{petName}{petBreed ? ` · ${petBreed}` : ""} · {config.label.replace(/ photo$/i, "")}</p>
          </div>
          {panelSnap !== "collapsed" ? <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-6">{controls}</div> : null}
        </section>
      </div>
    </div>
  );
